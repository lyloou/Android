...
public class ColorPickerView2 extends View {
	...
	public ColorPickerView2(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	protected void onDraw(Canvas canvas) {
		...
		// 画圆角长格（根据颜色数组）
		Bitmap bitmap = getBitmapByXfermode(mWidth, mHeight, paddingStart, paddingEnd, mColors, w3, h3);
	...
	private Bitmap getBitmapByXfermode(int width, int height, int paddingStart, int paddingEnd, int[] colors, float w3,
			float h3) {
		...
		// draw dst
		RectF rectF = new RectF(paddingStart, h3, width - paddingEnd, h3 * 2);
		...
		for (int i = 0; i < colors.length; i++) {
			rectF.set(paddingStart + i * w3, h3, paddingStart + (i + 1) * w3, h3 * 2);
